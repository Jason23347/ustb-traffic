...
3. 右键菜单：
   ...
   - **选项**：流量源、学号/密码（自服务）、轮询间隔、免费额度、边距、列间距、字体
...
选择 `https://zifuwu.ustb.edu.cn` 时，程序会用选项中的学号/密码做 HTTPS 登录并维护 Cookie（约每 2 小时刷新）；学号与密码明文保存在上述配置文件中。

## 数据来源

流量源可选：

- `202.204.48.82` / `202.204.48.66`：按固定间隔 GET 门户页，解析 HTML 中的 `flow='…'`（单位 KB）和 `NID`
- `https://zifuwu.ustb.edu.cn`：登录后 GET `/Self/dashboard`，解析「已用」字段（单位 M，换算为 KB）
